...
<head>
	...
	$(document).ready(
	    ...
	    function() {		
            var files = $( "#files" ).spinner({ min:0, max: 500 });
            var jobs = $( "#jobs" ).spinner({ min:0, max: 200 });
            var timeout = $( "#timeout" ).spinner({ min:0, max: 48*3600 });
            
             $("#loading").show();
             console.log('load servers/queues');
             $.post("getSE.asp", {}, function(msg){
                     if (msg.length){
                         msg = msg.trim().split(/\n/g);
                         jQuery.each(msg, function(i, line) {
                             line = line.split(/\t/);
                             $('#server').append($("<option></option>") .attr("value",line[0]).text(line[0])); 
                             $('#client').append($("<option></option>") .attr("value",line[1]).text(line[1])); 
                         });
                     }
                 });
    ...
    <table cellpadding="10" cellspacing="0" border="0" width="70%">  
        ...
        <tbody>
            <tr>
                ...
                <td>
                    <select id="type"> 
                        <option value="FAXcopy">FAXcopy</option>
                        <option value="read10pc">read 10% events</option>
                    </select>
                </td>
                <td>
                ...
                <td>
                    <input id="files" name="files" value="1" size="4"/>  
                </td>
                <td>
                    <input id="jobs" name="jobs" value="1" size="4"/>
                </td>
                <td>
                    <input id="timeout" name="timeout" value="1800" size="5"/>
                </td>
                <td>
