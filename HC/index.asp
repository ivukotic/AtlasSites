...
<head>
	...
	function extraSel(){
		$("#loading").show();
		var selSites=new Array();
		$('.csite').each( function (){ if (this.checked) {selSites.push($(this).val()); } }) ;
		// alert("getCPU menu");
		$.post("getExtras.asp", {project: $("#project").val(), from: $("#from").val(), to: $("#to").val() , sites: selSites.join()  }, 
			function(msg){ 
				// alert(msg);
				msg = msg.split(/\n/g);
				...
				$( "#ROOTextras" ).empty();
				$( "#STORAGEextras" ).empty();
				$( "#InputFilesExtras" ).empty();
				jQuery.each(msg, function(i, line) {
					line=$.trim(line);
					line = line.split(/\t/);
					if (line[0]=='cpu:') {
						$( "#CPUextras" ).append( '<input type="checkbox"  checked="yes" class="cCPUsel"  value="'+line[1]+'" />'+line[2]+'<br />' );
					}
					if (line[0]=='root:') {
						$( "#ROOTextras" ).append( '<input type="checkbox"  checked="yes" class="cROOTsel"  value="'+line[1]+'" />'+line[2]+'<br />' );
					}
					if (line[0]=='stor:') {
						$( "#STORAGEextras" ).append( '<input type="checkbox"  checked="yes" class="cSTORsel"  value="'+line[1]+'" />'+line[2]+'<br />' );
					}
					if (line[0]=='file:') {
						$( "#InputFilesExtras" ).append( '<input type="checkbox"  checked="yes" class="cFILEsel"  value="'+line[1]+'" />'+line[1]+'<br />' );
					}
				});
	...
	$(document).ready(function() {
		...
		$("#refresh").click(extraSel);
		$("#draw").click(gData);
		// $("#project").click(gSites);
		$('#toPlotX, #toPlotY').change(gData);
		$("#vsTime").click( function(){ 
			if ($("#vsTime").attr("checked")) { 
			    $('#toPlotX').attr('disabled', true); 
			    $('#g2avg').attr('checked', true); 
			    $('#g2projx').attr('disabled', true); 
			} else { 
			    $('#toPlotX').attr('disabled', false);
			    $('#g2projx').attr('disabled', false); 
			} 
		});
		...
		// function gSites(){ 
		// 	$("#loading").show();
		// 	$.post("getSites.asp", {project: $("#project").val(), from: $("#from").val(), to: $("#to").val()  }, 
		// 		function(msg){ 
		// 			msg=$.trim(msg);
		// 			msg = msg.split(/\t/g);
		// 			$( "#sites" ).empty();
		// 			jQuery.each(msg, function(i, line) {
		// 				$( "#sites" ).append( '<input type="checkbox" class="csite" onclick="extraSel()" value="'+line+'" />'+line+'<br />' );
		// 			});
		// 			
		// 			$("#loading").hide();
		// 			
		...
		function gData(){
			...
			if ($("#dLine").attr("checked")){options.chart.type='line';}else{options.chart.type='scatter';}
			var xax=$("#toPlotX").val();
			if ($("#vsTime").attr("checked")) {
			...
			var selCPU=new Array(); $('.cCPUsel').each( function (){ if (!this.checked) {selCPU.push($(this).val()); } }) ;
			var selROOT=new Array(); $('.cROOTsel').each( function (){ if (!this.checked) {selROOT.push($(this).val()); } }) ;
			var selSTOR=new Array(); $('.cSTORsel').each( function (){ if (!this.checked) {selSTOR.push($(this).val()); } }) ;
			var selFILE=new Array(); $('.cFILEsel').each( function (){ if (!this.checked) {selFILE.push($(this).val()); } }) ;
			$.post("getData.asp", {project: $("#project").val(), toplotX: xax, toplotY: $("#toPlotY").val(), from: $("#from").val(), to: $("#to").val(), sites: selSites.join(), cpus:selCPU.join() , roots:selROOT.join(), stors:selSTOR.join(), ifile:selFILE.join()  }, 
				function(msg){ 
					var avers= new Array();
                    var maxx=-9999;
                    var maxy=-9999;
...
<body>
	...
	<div id="leftcolumn" >
		...
		x: <select id="toPlotX">
			...
			<option value="vmem">VMEM [kb]</option>
			<option value="rss">RSS [kb]</option>
			...
			<option value="root.events">events</option>
			<option value="root.reads">root reads</option>
			<option value="root.ttc">root ttc [bytes]</option>
			<option value="root.totalsize">root size [bytes]</option>
			<option value="root.zipsize">root zipsize [bytes]</option>
			<option value="root.bytesread">root bytesread [bytes]</option>
		...
		<div style="clear:both">
    		<button id="refresh"  >Refresh</button><button id="draw">Draw</button><button id="bANNO"  >Annotations</button>
		</div>
		<div id="accordion">
			<h3><a href="#">Sites</a></h3>
			<div id="sites">
			</div>
			<h3><a href="#">CPUs</a></h3>
			<div id="CPUextras">
			</div>
			<h3><a href="#">ROOT versions</a></h3>
			<div id="ROOTextras">
			</div>
			<h3><a href="#">Storage</a></h3>
			<div id="STORAGEextras">
			</div>	
			<h3><a href="#">Input files</a></h3>
			<div id="InputFilesExtras">
			</div>
		</div>
	</div>
	...
	<div id="dialog" title="ATLAS HammerCloud IO tests" style="text-align:left">	
        <h3>Idea</h3>
        Where are several reasons for this test framework:
        <ul>
        	<li>We need to measure IO performance of our files at most of the ATLAS resources.</li>
        	<li>We want to test impact of any changes we will make on: ROOT file organization/setting, ROOT/Athena version used, way we use the files.</li>
        ...
        <ul>
            <li><b>10% no cache</b> reads random sequential 10% of events with TTreeCache disabled. </li>
            <li><b>10% default cache</b> reads random sequential 10% of events with TTreeCache set to default value of 30MB. </li>
            <li><b>TOPJET</b> should be a reasonably realistic TOPJET analysis code.</li>
        </ul> 
