...
var procstep = new String(Request("procstep"));
var stream  = new String(Request("stream")); 

// Response.Write(usemu+'\n');
// Response.Write(useid+'\n');

sel="SELECT runnumber, cast(cputime as number) ";
if (usemu==1) sel+=', mu ';
if (useid==1 || usemu==1) 
...
sel1="SELECT UNIQUE category FROM "+(sel.split("FROM")[1]);
// Response.Write(sel1);
...
try{
	...
	for (s in iselcbs){
		que= sel + " AND category='"+iselcbs[s]+"' order by runnumber asc";
        // Response.Write(que + '\n');
